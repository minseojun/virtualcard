...
def calculate_risk(payment_amount, card, site, country):
    ...
    risk += random.choice([0.2, 0.3, 0.4])
    sampled_reasons = random.sample(risk_factors, k=random.randint(1,3))
    reasons.extend(sampled_reasons)
    if card['restricted'] and card.get('allowed_sites') and site not in card['allowed_sites']:
        risk += 0.3
        reasons.append('허용되지 않은 사이트')
    risk = min(risk, 1.0)
    return risk, list(set(reasons))  # 중복 제거
